...
SYSTEM_TYPES = ['公安系统','法院系统','检察院系统','司法行政','纪委监委','党委系统','政府办公室','发改系统','财政系统','审计系统','市场监管系统','统计系统','教育系统','卫健系统','住建系统','交通系统','农业农村系统','人社系统','民政系统','水利系统','自然资源系统','生态环境系统','乡镇街道']
...
def assign_scores(city, system_type, education, recruit_num, political):
    ...
    elif system_type in ['纪委监委','发改系统','财政系统']:
        salary_base += 0.5
    if '市直' == '市直':
        salary_base += 0.3
    ...
    # 发展前景
    prospect_base = 5.0
    if system_type in ['党委系统','政府办公室','纪委监委']:
        prospect_base = 8.0
    elif system_type in ['发改系统','财政系统']:
        prospect_base = 7.8
    elif system_type in ['公安系统','法院系统','检察院系统']:
        prospect_base = 7.5
    elif system_type in ['教育系统','卫健系统','住建系统','交通系统','审计系统','市场监管系统']:
        prospect_base = 7.0
    elif system_type in ['统计系统','自然资源系统','生态环境系统','水利系统','人社系统','民政系统','农业农村系统']:
        prospect_base = 6.5
...
def get_provincial_sys_type(unit_name):
    """根据省级单位名称自动判断系统类型"""
    if '省委' in unit_name:
        return '党委系统'
    if '纪委' in unit_name:
    ...
    if '司法' in unit_name:
        return '司法行政'
    if '办公厅' in unit_name or '办公室' in unit_name or '机关事务' in unit_name:
        return '政府办公室'
    if '发展' in unit_name or '改革' in unit_name:
        return '发改系统'
    if '财政' in unit_name:
        return '财政系统'
    if '审计' in unit_name:
    ...
    if '统计' in unit_name:
        return '统计系统'
    if '市场监督' in unit_name:
        return '市场监管系统'
    if '教育' in unit_name:
        return '教育系统'
    if '卫生健康' in unit_name or '医疗' in unit_name:
        return '卫健系统'
    if '住建' in unit_name or '住房' in unit_name:
        return '住建系统'
    if '交通' in unit_name or '运输' in unit_name:
        return '交通系统'
    if '农业' in unit_name or '农村' in unit_name:
        return '农业农村系统'
    if '人社' in unit_name or '人力' in unit_name or '社会保障' in unit_name:
        return '人社系统'
    if '民政' in unit_name:
        return '民政系统'
    if '水利' in unit_name:
    ...
    if '自然' in unit_name:
        return '自然资源系统'
    if '生态' in unit_name or '环境' in unit_name:
        return '生态环境系统'
    if '科学' in unit_name or '技术厅' in unit_name:
        return '科技系统'
    if '工业和信息化' in unit_name or '工信' in unit_name:
        return '工信系统'
    if '商务' in unit_name:
        return '商务系统'
    if '文化和旅游' in unit_name or '体育' in unit_name:
        return '文旅体育系统'
    if '应急' in unit_name:
        return '应急管理系统'
    if '退役' in unit_name:
    ...
    if '信访' in unit_name:
        return '信访系统'
    if '粮' in unit_name or '储备' in unit_name:
        return '粮食储备系统'
    return '政府办公室'
...
def seed_database():
    ...
    # 市级单位 → 系统类型 映射（细分职能部门，不再全部归为"政府办公室"）
    UNIT_SYS_MAP = {
        '公安局': '公安系统', '法院': '法院系统', '检察院': '检察院系统',
        '纪委监委': '纪委监委', '司法局': '司法行政',
        '发改委': '发改系统', '财政局': '财政系统', '审计局': '审计系统',
        '市场监管局': '市场监管系统', '统计局': '统计系统',
        '教育局': '教育系统', '卫健委': '卫健系统',
        '住建局': '住建系统', '交通局': '交通系统',
        '农业农村局': '农业农村系统', '人社局': '人社系统',
        '民政局': '民政系统', '水利局': '水利系统',
        '自然资源局': '自然资源系统', '生态环境局': '生态环境系统',
    }
    ...
    for year in years:
        ...
        for pu in provincial_units:
            if random.random() < 0.75:  # 75%概率生成省直岗位
                pos_id += 1
                sys_type = get_provincial_sys_type(pu)
        ...
        for city_name, city_data in CITIES.items():
            ...
            for unit_key, unit_template in city_units_templates.items():
                ...
                sys_type = UNIT_SYS_MAP.get(unit_key, '政府办公室')
    ...
    for i, entry in enumerate(entries):
        ...
        if sys_type in ['党委系统','政府办公室','纪委监委','发改系统','财政系统']:
